<?php
    ...
    if($_POST['email'] != "" && $_POST['password'] != "" && $_POST['name'] != "" && $_POST['surname'] != "" && $_POST['username'] != "" && $_POST['born'] != "" && $_POST['conf-password']){
      if($_POST['conf-password'] == $_POST['password']){
        ...
        $query = 'INSERT INTO utenti '.'(`utente_id`, `utente_email`, `utente_password`, `utente_nome`, `utente_cognome`, `utente_username`, `utente_nascita`) VALUES (NULL,"'.
            ...
            $rs = mysqli_query($link, $query);
            mysqli_close($link);
            header('Location: /index.php?registrazione=1');
          }
        }
        }else{
            mysqli_close($link);
          ...
          mysqli_close($link);
          header('Location: /registrazione.php?msg=campi');
        }
 ?>
